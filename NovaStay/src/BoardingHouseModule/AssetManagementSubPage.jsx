...
export default function AssetManagementSubPage() {
                ...
                <div className="bg-[#16171E] border border-[#2C2D35] rounded-sm p-4 mb-6">
                    <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
                        ...
                        <button
                            ...
                            className="flex items-center justify-center gap-1.5 bg-gradient-to-r from-[#A98446] to-[#D4AF37] text-black text-[11px] font-bold px-4 py-2.5 rounded-sm hover:opacity-90 transition-opacity uppercase tracking-wider whitespace-nowrap h-[36px]"
                        >
                ...
                {filteredAssets.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {filteredAssets.map((asset) => (
                            <div key={asset.id} className="bg-[#16171E] border border-[#2C2D35] hover:border-[#414352] rounded-sm transition-all duration-300 flex flex-col justify-between group relative overflow-hidden shadow-xl">

                                {/* KHỐI HÌNH ẢNH MINH HỌA CAO CẤP */}
                                <div className="h-44 w-full relative overflow-hidden bg-slate-900 border-b border-[#2C2D35]/50 shrink-0">
                                    <img
                                        src={asset.image}
                                        alt={asset.name}
                                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-85 group-hover:opacity-100"
                                    />
                                    <div className="absolute inset-0 bg-gradient-to-t from-[#16171E] via-transparent to-transparent"></div>
                                ...
                                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                                    <div>
                                        <div className="flex justify-between items-center mb-1">
                                            <span className="text-[10px] font-mono text-[#8A8D98] tracking-wider">{asset.id}</span>
                                            <span className="text-[10px] text-[#C5A880] font-medium uppercase bg-[#C5A880]/5 px-2 py-0.5 border border-[#C5A880]/20 rounded-sm">
                                                {asset.category}
                                            </span>
                                        </div>
                                        <h3 className="text-sm font-medium text-white group-hover:text-[#C5A880] transition-colors line-clamp-1">{asset.name}</h3>
                                        <p className="text-[11px] text-[#8A8D98] mt-1.5 font-light line-clamp-2 leading-relaxed">{asset.description}</p>
                                    </div>

                                    <div className="pt-3 border-t border-[#2C2D35]/40 flex justify-between items-center text-xs">
                                        <span className="text-[#5A5C66] uppercase tracking-wider text-[9px] flex items-center gap-1">
                                            <Package size={11} /> Tổng số lượng cấp phát
                                        </span>
                                        <span className="text-base font-normal font-mono text-white tracking-wide">
                                            {asset.quantity} <span className="text-[10px] text-[#5A5C66] font-sans">thiết bị</span>
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="p-3 bg-[#1B1C24] border-t border-[#2C2D35] flex justify-between items-center text-[11px] tracking-wider">
                                    <button className="text-[#8A8D98] hover:text-[#C5A880] flex items-center gap-1 transition-colors px-2 py-1.5 text-[10px] uppercase font-mono">
                                        <Info size={11} /> Lịch sử cấp phát
                                    </button>
                                    <button className="flex items-center justify-center gap-1 text-[#C9CBD3] hover:text-white transition-colors font-semibold uppercase bg-[#1F212A] border border-[#2C2D35] px-3 py-1.5 rounded-sm text-[10px] tracking-widest">
                    ...
                    <div className="py-16 text-center border border-dashed border-[#2C2D35] bg-[#16171E] rounded-sm flex flex-col items-center justify-center min-h-[300px]">
                        <Package size={32} className="text-[#2C2D35] mb-2" />
                        <h3 className="text-sm font-medium text-white tracking-wide">Không tìm thấy tài sản phù hợp</h3>
                    </div>
                )}
            </div>
                    ...
                    <div className="relative bg-[#16171E] border border-[#3E3F4A] max-w-md w-full p-6 shadow-2xl rounded-sm transform transition-all animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="flex justify-between items-center border-b border-[#2C2D35] pb-4 mb-5">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-sm font-semibold tracking-widest text-white uppercase">Khai Báo Tài Sản Nội Thất</h3>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-[10px] tracking-widest text-[#8A8D98] mb-1.5 uppercase font-medium">Tên tài sản / Thiết bị *</label>
                                <input
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-[10px] tracking-widest text-[#8A8D98] mb-1.5 uppercase font-medium">Danh mục nhóm</label>
                                ...
                                <div>
                                    <label className="block text-[10px] tracking-widest text-[#8A8D98] mb-1.5 uppercase font-medium">Số lượng nhập kho *</label>
                                    <input
                            ...
                            <div>
                                <label className="block text-[10px] tracking-widest text-[#8A8D98] mb-1.5 uppercase font-medium">Đường dẫn ảnh minh họa (URL)</label>
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full bg-[#1F212A] border border-[#2C2D35] text-xs px-3 py-2.5 pl-9 text-white rounded-sm focus:outline-none focus:border-[#C5A880] transition-colors placeholder-[#5A5C66]"
                                    />
                                    <ImageIcon size={13} className="absolute left-3 top-3 text-[#5A5C66]" />
                                </div>
                            </div>

                            <div>
                                <label className="block text-[10px] tracking-widest text-[#8A8D98] mb-1.5 uppercase font-medium">Mô tả đặc tính tài sản</label>
                                <textarea
                                    rows="3" placeholder="Thông số kỹ thuật, vị trí phân chia phòng..."
                                    value={formData.description}
                                    onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                                    className="w-full bg-[#1F212A] border border-[#2C2D35] text-xs p-3 text-white rounded-sm focus:outline-none focus:border-[#C5A880] transition-colors placeholder-[#5A5C66] resize-none"
                                />
                            </div>
